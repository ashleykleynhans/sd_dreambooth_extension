...
def main(class_gen_method: str = "Native Diffusers", user: str = None) -> TrainResult:
    ...
    def inner_loop(train_batch_size: int, gradient_accumulation_steps: int, profiler: profile):
        ...
        logger.info(f"  Num batches each epoch = {len(train_dataset) // train_batch_size}")
        ...
        logger.info(f"  Total optimization steps = {sched_train_steps}")
        logger.info(f"  Total training steps = {max_train_steps}")
        logger.info(f"  Resuming from checkpoint: {resume_from_checkpoint}")
        logger.info(f"  First resume epoch: {first_epoch}")
        logger.info(f"  First resume step: {resume_step}")
        logger.info(f"  Lora: {args.use_lora}, Optimizer: {args.optimizer}, Prec: {precision}")
        logger.info(f"  Gradient Checkpointing: {args.gradient_checkpointing}")
        logger.info(f"  EMA: {args.use_ema}")
        logger.info(f"  UNET: {args.train_unet}")
        logger.info(f"  Freeze CLIP Normalization Layers: {args.freeze_clip_normalization}")
        logger.info(f"  LR{' (Lora)' if args.use_lora else ''}: {learning_rate}")
        if stop_text_percentage > 0:
            logger.info(f"  Tenc LR{' (Lora)' if args.use_lora else ''}: {txt_learning_rate}")
        logger.info(f"  LoRA Extended: {args.use_lora_extended}")
        logger.info(f"  V2: {args.v2}")
